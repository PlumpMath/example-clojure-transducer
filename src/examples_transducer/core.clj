...
(defn -main
  ...
  ;; 즉, "(comp f1 f2)"는 "(fn [& args] (f1 (apply f2 args)))"를 반환한다.
  ...
  ;; 위 코드의 "((map #(+ 1 %)) conj)"는 "(fn [result input] (conj result (+ 1 input)))"와 동일하다.
  ;; 즉, step 함수는 "f"에 입력값을 넣고 반환된 결과를 "rf"로 전달한다.
  ;; 다시 말해, "rf"로 "f"의 결과 값이 전달된다.
  ...
  ;; 이제, 아래 코드와 같이 처음 transduce를 사용한 코드와 비슷하게 filter를 함수 파이프 라인에 추가하였다.
